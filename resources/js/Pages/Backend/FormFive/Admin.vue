<template>
    <div class="flex justify-between items-center text-color">
        <div>
            ...
            <p class="text-caption">Form-5 (i) Submitted by all departments</p>
            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-five-one.all')"
    ...
    <div class="column">
        <q-list>
            <q-item class="q-pl-none q-ml-none" v-for="(item,index) in props.list.data">
                ...
                <q-item-section>
                    <q-item-label>
                        Uploaded By {{item?.user?.name}} for
                        <span class="text-bold">{{item?.department?.name}}</span>
                    </q-item-label>
...
const doFilter=value=>{
    router.get(route('form-five-one.all'),{
