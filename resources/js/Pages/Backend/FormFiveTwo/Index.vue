<template>
    <div class="flex justify-between items-center text-color">
        <div>
            <p class="page-header">Form Five (ii)</p>
            <p class="text-caption">Beneficiaries Register</p>

            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-five-two.index')"
                                  label="Form Five (ii)"/>
            </q-breadcrumbs>
        </div>
        <q-btn color="accent" @click="$inertia.get(route('form-upload-five-two.create'))" label="Upload New"/>
    </div>
    <br/>
    <div class="column">
        <q-list>
