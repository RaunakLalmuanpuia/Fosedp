<template>
    <div class="bg-white">
        <div class="flex justify-between items-center text-color">
            <div>
                <p class="page-header">Edit Application : {{data?.head_of_family}}</p>
                <q-breadcrumbs>
                    <q-breadcrumbs-el @click="$inertia.get(route('dashboard'))" label="Dashboard" icon="o_dashboard" />
                    <q-breadcrumbs-el @click="$inertia.get(route('application.received'))" label="Inbox" />
                    <q-breadcrumbs-el label="Edit Application" />
                </q-breadcrumbs>
            </div>
        </div>
        <br/>
        <q-form @submit="submit" class="row q-col-gutter-sm">
            ...
            <div class="col-xs-12 col-sm-6">
                <q-select v-model="form.constituency"
                          ...
                          :error-message="form?.errors?.constituency"
                          :rules="[
            ...
            <div class="col-xs-12 col-sm-6">
                <q-select v-model="form.trade"
                          ...
                          :error-message="form?.errors?.trade"
                          :rules="[
            ...
            <div class="col-xs-12">
                <div class="flex q-gutter-sm">
                    <q-btn type="submit" color="accent" label="Update"/>
...
const q = useQuasar();
const props = defineProps([ 'data','districts','departments'])
const form = useForm({
    head_of_family: props.data?.head_of_family,
    mobile: props.data?.mobile,
    address: props?.data?.address,
    epic_no: props?.data?.epic_no,
    district: !!props?.data?.district ? ({value:props.data?.district?.id,label:props?.data?.district?.name}) :null,
    constituency: !!props?.data?.constituency ? ({value:props.data?.constituency?.id,label:props?.data?.constituency?.name}) :null,
    department: !!props?.data?.department ? ({value:props.data?.department?.id,label:props?.data?.department?.name}) :null,
    trade: !!props?.data?.trade ? ({value:props.data?.trade?.id,label:props?.data?.trade?.name}) :null,
    subTrade: !!props?.data?.sub_trade ? ({value:props.data?.sub_trade?.id,label:props?.data?.sub_trade?.name}) :null,
    bank_name: props.data?.bank_account?.bank_name,
    branch_name: props.data?.bank_account?.branch_name,
    ac_holder: props.data?.bank_account?.ac_holder,
    ac_no: props.data?.bank_account?.ac_no,
    ifsc: props.data?.bank_account?.ifsc
})
...
const submit=e=>{
    ...
    })).put(route('application.update',props.data.id),{
...
onMounted(()=>{
    onDistrictSelect(form.district);
    form.constituency=!!props?.data?.constituency ? ({value:props.data?.constituency?.id,label:props?.data?.constituency?.name}) :null;
    onDepartmentSelect(form.department);
    form.trade=!!props?.data?.trade ? ({value:props.data?.trade?.id,label:props?.data?.trade?.name}) :null;
    onTradeSelect(form.trade);
    form.subTrade=!!props?.data?.sub_trade ? ({value:props.data?.sub_trade?.id,label:props?.data?.sub_trade?.name}) :null;
})
