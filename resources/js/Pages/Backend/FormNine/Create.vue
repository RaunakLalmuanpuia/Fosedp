<template>
    <div class="flex justify-between items-center text-color">
        <div>
            <p class="page-header">Upload Form 9</p>
            <p class="text-caption">Closure Certificate</p>
            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-nine.index')"
                                  label="Form 9"/>
                <q-breadcrumbs-el label="Upload Form 9 - Closure Certificate"/>
            </q-breadcrumbs>
        </div>
    </div>
    <br/>
...
const submit=e=>{
    form.transform(data=>({department_id:data.department.value,...data}))
        .post(route('form-upload.store','form-nine'),{
