<template>
    <div class="flex justify-between items-center text-color">
        <div>
            <p class="page-header">Form 10</p>
            <p class="text-caption">(FOSEDP kalpui tha, chawimawi tlak recommend form)</p>
            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-ten.index')"
                                  label="Form 10"/>
            </q-breadcrumbs>
        </div>
        <q-btn color="accent" @click="$inertia.get(route('form-upload-ten.create'))" label="Upload New"/>
    </div>
    <br/>
    <div class="column">
        <q-list>
            <q-item class="q-pl-none q-ml-none" v-for="(item,index) in props.list">
                ...
                <q-item-section>
                    <q-item-label>Uploaded By <span class="text-bold">{{item?.user?.name}}</span>  for <span class="text-bold">{{item?.department?.name}}</span> </q-item-label>
