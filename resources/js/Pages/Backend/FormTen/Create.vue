<template>
    <div class="flex justify-between items-center text-color">
        <div>
            <p class="page-header">Upload Form 10</p>
            <p class="text-caption">FOSEDP kalpui tha, chawimawi tlak recommend na form</p>
            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-ten.index')"
                                  label="Form 10"/>
                <q-breadcrumbs-el label="Upload Form 10 - (FOSEDP kalpui tha, chawimawi tlak recommend form)"/>
            </q-breadcrumbs>
        </div>
    </div>
    <br/>
...
const submit=e=>{
    form.transform(data=>({department_id:data.department.value,...data}))
        .post(route('form-upload.store','form-ten'),{
