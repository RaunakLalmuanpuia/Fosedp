<template>
    <div class="flex justify-between items-center text-color">
        <div>
            <p class="page-header">Form 11</p>
            <p class="text-caption">Monitoring</p>
            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-eleven.index')"
                                  label="Form 11"/>
            </q-breadcrumbs>
        </div>
        <q-btn color="accent" @click="$inertia.get(route('form-upload-eleven.create'))" label="Upload New"/>
    </div>
    <br/>
    <div class="column">
        <q-list>
