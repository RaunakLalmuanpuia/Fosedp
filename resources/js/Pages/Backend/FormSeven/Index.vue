<template>
    <div class="flex justify-between items-center text-color">
        <div>
            <p class="page-header">Form 7</p>
            <p class="text-caption">Inspection Report</p>
            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-seven.index')"
                                  label="Form 7"/>
            </q-breadcrumbs>
        </div>
        <q-btn color="accent" @click="$inertia.get(route('form-upload-seven.create'))" label="Upload New"/>
    </div>
    <br/>
    <div class="column">
        <q-list>
