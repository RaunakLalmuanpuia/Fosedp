<template>
    <div class="flex justify-between items-center text-color">
        <div>
            ...
            <p class="text-caption">Form-7 (Inspection Report) Submitted by all departments</p>
            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-seven.all')"
                                  label="Form 7 - (Inspection Report)"/>
            </q-breadcrumbs>
        </div>
...
const doFilter=value=>{
    router.get(route('form-seven.all'),{
