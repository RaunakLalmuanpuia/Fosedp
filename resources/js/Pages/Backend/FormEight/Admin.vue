<template>
    <div class="flex justify-between items-center text-color">
        <div>
            ...
            <p class="text-caption">Form-8 (Completion Certificate) Submitted by all departments</p>
            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-eight.all')"
                                  label="Form 8 - (Completion Certificate)"/>
            </q-breadcrumbs>
        </div>
...
const doFilter=value=>{
    router.get(route('form-eight.all'),{
