<template>
    <div class="flex justify-between items-center text-color">
        <div>
            <p class="page-header">Form 8</p>
            <p class="text-caption">(Completion Certificate)</p>
            <q-breadcrumbs>
                <q-breadcrumbs-el :href="route('dashboard')" icon="o_dashboard" label="Dashboard"/>
                <q-breadcrumbs-el :href="route('form-upload-eight.index')"
                                  label="Form 8 "/>
            </q-breadcrumbs>
        </div>
        <q-btn color="accent" @click="$inertia.get(route('form-upload-eight.create'))" label="Upload New"/>
    </div>
    <br/>
    <div class="column">
        <q-list>
