<template>
    <div>
        <slot name="trigger">
            <q-btn @click="open=true" label="Forward to Executive" color="primary" no-caps/>
        </slot>

        <q-dialog v-model="open">
            <q-card style="width: 540px" flat class="">
                <div class="flex justify-between items-center q-pl-md">
                    <p class="text-lg q-ma-none q-my-sm">Forward To Executive Board</p>
...
const fetchPlanningUsers=e=>{
    ...
    axios.get(route('option.executive-users'))
    .then(res=>{
